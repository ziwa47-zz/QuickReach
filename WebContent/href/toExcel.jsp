...
<jsp:useBean id="toexcel" class="tw.iii.qr.stock.CDBtoExcel" scope="page" />
<jsp:useBean id="toexcelidp" class="tw.iii.qr.stock.CDBtoExcelIDP" scope="page" />
...
<%
	...
	if ("1idp".equals(request.getParameter("excel"))) {
		try {
			String[] path = toexcelidp.物流匯出格式();
			String pa = path[0];
			String filename = path[1];
	...
	if ("2idp".equals(request.getParameter("excel"))) {
		try {
			String path[] = toexcelidp.日結表();
	...
	if ("3idp".equals(request.getParameter("excel"))) {
		try {
			//日出貨報表
			String[] path = toexcelidp.dailyBalanceSheetExcel();
	...
	if ("4idp".equals(request.getParameter("excel"))) {
		try {
			String[] path = (String[]) session.getAttribute("pathsent");
	...
	if ("5idp".equals(request.getParameter("excel"))) {
		try {
			String[] path = null;
			if (session.getAttribute("pathpick") == null || "".equals(session.getAttribute("pathpick"))) {
			} else {
	...
	if ("6idp".equals(request.getParameter("excel"))) {
		try {
			String[] path = (String[]) session.getAttribute("pathcoll");
			String pa = path[0];
			String filename = path[1];
