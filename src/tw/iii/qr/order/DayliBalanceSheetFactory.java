...
public LinkedList<COrders> dayliBalanceSheet(HttpServletRequest request,HttpServletResponse response , Connection conn) throws ClassNotFoundException, Exception {
	...
	DataBaseConn jdbc = new DataBaseConn();
		...
		while(rs.next()){
			...
			order.COrderDetail.setSKU(rs.getString(3));
			order.COrderDetail.setProductName(rs.getString(4));
