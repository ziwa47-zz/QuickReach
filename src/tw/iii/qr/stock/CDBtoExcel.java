...
public class CDBtoExcel {
	...
	public String[] 日結表() throws IllegalAccessException, ClassNotFoundException, SQLException, Exception {
		...
		String strsql = "select distinct m.ebayNO, m.ebayItemNO , d.SKU, d.productName,d.qty, m.guestAccount,"
				 ...
				 +" orders_detail as d on m.QR_id = d.QR_id inner join"
				 +" order_recieverinfo as r on m.QR_id = r.QR_id inner join"
				 +" shippinglog as s on m.QR_id = r.QR_id  inner join"
				 +" product as p on d.SKU  = p.SKU "
		...
		FileOutputStream out = new FileOutputStream(fsv.getHomeDirectory()+File.separator +"QRexcel"+File.separator+ date + "Daily.xlsx");
		wb.write(out);
		rs.close();
		ps.close();
		conn.close();
...
//				if(!"EMS".equals(wb.getNameAt(j))|| !"A&P".equals(wb.getNameAt(j))){
//					wb.removeName(j);
//				}
//			}
	...
	public void get揀貨單2() throws IllegalAccessException, ClassNotFoundException, SQLException, Exception {
		...
		FileSystemView fsv = FileSystemView.getFileSystemView();
		FileOutputStream out = new FileOutputStream(
				fsv.getHomeDirectory() + File.separator + "QRexcel" + File.separator + date + "揀貨單2.xlsx");
		wb.write(out);
		rs.close();
		ps.close();
		conn.close();
	...
	public String[] 物流匯出格式() throws IllegalAccessException, ClassNotFoundException, SQLException, Exception {
		...
		FileSystemView fsv = FileSystemView.getFileSystemView();
		FileOutputStream out = new FileOutputStream(
				fsv.getHomeDirectory() + File.separator + "QRexcel" + File.separator + date + "test.xlsx");
		wb.write(out);
		wb.close();
		rs.close();
		ps.close();
		conn.close();
		String[] path = { fsv.getHomeDirectory() + File.separator + "QRexcel" + File.separator, date + "test.xlsx" };
